	...
	for($i = 0; $i < 32; $i++){
	    ...
	    if($i > 0){			// 屏蔽第一帧
		    for ($k = 0; $k < strlen($authstr); $k++) { 
				...
				$float_top = rand(-4,4);	//文字动画跳跃高度在-4~4像素
				//$top + $float_top 随机了文字位置的高度，看上去图片就有动画效果了；
	    ...
	    imagegif($image);		//绘制图片
	    imagedestroy($image);		//清除画板
	    $imagedata[] = ob_get_contents();		//把当前帧图存入缓冲区，后续合并成动画需要
	    ob_clean();
	    ++$i;
	}
	// Header ('Content-type:image/gif');  
 //    echo $imagedata[15];
	$gif = new GIFEncoder(array($imagedata[15]));  
	Header ('Content-type:image/gif');  
